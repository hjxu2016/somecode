...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 30 19:57:40 2017
...
TRAIN_WSI_PATH_Tumor = '/media/hjxu/Elements/Camelyon16/Training/Tumor'
TRAIN_WSI_PATH_Normal = '/media/hjxu/Elements/Camelyon16/Training/N'
train_Tumor_wsi_paths = glob.glob(os.path.join(TRAIN_WSI_PATH_Tumor, '*.tif'))
train_Tumor_wsi_paths.sort()
train_Normal_wsi_paths = glob.glob(os.path.join(TRAIN_WSI_PATH_Normal, '*.tif'))
train_Normal_wsi_paths.sort()

Tumor_pridict_path = '/media/hjxu/Elements/Camelyon16_Result/tumor_pre_for_ROC/'
Normal_pridict_path = '/media/hjxu/Elements/Camelyon16_Result/nor_pre_ROC/'
f_test = '/home/hjxu/breast_project/post_process/features/feature_train.csv'
...
i=1
for wsi_path in train_Tumor_wsi_paths:
    print 'train:',i
    i = i+1
    wsi_name = get_filename_from_path(wsi_path)
    image_open = get_image_open(wsi_path)
    
    heatmap_prob_path = Tumor_pridict_path + wsi_name+'_predict.tif'
...
for wsi_path in train_Normal_wsi_paths:
    print 'test:',i
    i = i+1
    wsi_name = get_filename_from_path(wsi_path)
    image_open = get_image_open(wsi_path)
    
    heatmap_prob_path = Normal_pridict_path + wsi_name+'_predict.tif'
    heatmap_prob = cv2.imread(heatmap_prob_path)
    features = extract_features(wsi_name,heatmap_prob, image_open)
    if 'umor' in wsi_name:
        features += [1]
